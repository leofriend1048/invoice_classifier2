...
function FilterAmount() {
  const [minAmount, maxAmount] = React.useMemo(() => {
    // Replace any usage of transactions with a prop or context that provides invoices from Supabase, or refactor to not depend on local mock data.
    return [0, 7000]
  }, [])
  ...
  const distributionData = React.useMemo(() => {
    ...
    for (let i = 0; i < 100; i++) {
      ...
      const binIndex = Math.min(
        Math.floor((amount - minAmount) / binSize),
        numBins - 1,
      )
      bins[binIndex]++
    }
